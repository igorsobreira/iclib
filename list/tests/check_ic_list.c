...
START_TEST (nth_should_return_element_data_at_that_position_or_NULL_if_out_of_bounds)
{
  int num1 = 1, num2 = 2;
  ic_list *mylist = ic_list_new();
  ...
  fail_unless(*(int *)ic_list_nth_data(mylist, 0) == num1);
  fail_unless(*(int *)ic_list_nth_data(mylist, 1) == num2);
  fail_unless(ic_list_nth_data(mylist, 2) == NULL);
  fail_unless(ic_list_nth_data(mylist, 10) == NULL);
